...
inline float3	reflect(const float3	i,
						const float3	n)
{
	return (i - n * 2.0f * dot(i, n));
}

inline float3	refract(const float3	i,
						const float3	n,
						const float		m1,
						const float		m2)
{
	const float		eta = m1 / (float)m2;
	const float		c1 = (dot(i, n) < 0.0f) ? -dot(i, n) : dot(i, n);
	const float		cs1 = 1 - (m1 / (float)m2) * (m1 / (float)m2);
	float			cs2 = cos(c1);
	cs2 = 1 - cs2 * cs2;
	const float		c2 = sqrt(cs1 * cs2);

	const float3	t = eta * i + (c1 - c2) * n;
	return (t);
}
						...
						__global uint				*skybox)
{
	...
	if (r.t < MAX_DEPTH) {
		if (r.att.w == 1)
			...
			if (r.att.x <= 0.2){
				r.color += metallic(r, d) * lighting(r, d) * (r.att.y + (0.2f - r.att.x * 0.2f));
			}
		}
	}
