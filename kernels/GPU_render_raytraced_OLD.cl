...
typedef struct		s_brdf
{
	...
	float			light_pow;
	float			light_dist;
//	float3			spot_dir;			//SPOT_LIGHT ONLY
	float4			att;
	int				id;
	t_data			d;
}					t_brdf;
...
{
	const float		nseg = d.obj[id + 7];
	float3			v1, v2, v3, e1, e2, p, t_vec, q;
	float			u, v, t_dist;
	float			det, idet;
	int				offset;
	...
	for (int i = 0; i < nseg; i++){
		offset = i * 9;
		v1 = (float3)(d.obj[id + offset + 8], d.obj[id + offset + 9], d.obj[id + offset + 10]);
		v2 = (float3)(d.obj[id + offset + 11], d.obj[id + offset + 12], d.obj[id + offset + 13]);
		v3 = (float3)(d.obj[id + offset + 14], d.obj[id + offset + 15], d.obj[id + offset + 16]);
		e1 = v2 - v1;
		e2 = v3 - v1;
		p = cross(r->dir, e2);
		det = dot(p, e1);
		if (det > -0.00001f || det < 0.00001f)
		{
			...
			u = dot(p, t_vec) * idet;
			v = dot(q, r->dir) * idet;
			t_dist = dot(q, e2) * idet;
			if (u >= 0 && v >= 0 && u + v <= 1 && t_dist > MIN_DIST)
			{
				r->t = t_dist;
				r->id = id;
				const float3  n = cross(e1, e2);
										...
										t_data			d)
{
	...
	float 			t0 = 0.0;
	float 			t1 = 0.0;
	float 			k = tan(radius * (10 * 3.141592f) / 180.0f);
	float 			p = 1.0f + k * k;
	float 			tmp = dot(r->dir, dir);
	float 			tmp2 = dot(r->o - center, dir);
	float 			m = dot(r->dir, dir * t) + dot(r->o - center, dir);
...
__inline float	check_hit(		t_ray*			r,
								t_data			d)
{
	...
	if (r->t < MAX_DEPTH)
	{
		r->att.x = d.obj[r->id + 3];
		r->att.y = d.obj[r->id + 4];
...
{
	t_ray					lr = {.dir = b.l, .o = b.p, .t = MAX_DEPTH};
							...
							t_data			d)
{
	...
	for (int i = 0; i < total_lights; i++){
		...
		// SPOT LIGHT VERIFICATION AND ASSIGNEMENT
		float		fallof = b.light_pow / (float)b.light_dist;
//		fallof = fallof < 0.0001 ? log(b.l : fallof;
...
__inline float3	metallic(	t_ray			r,
							t_data			d)
{
	float3			final_col =	r.color * 0.3f;
	t_ray 			ref = {.dir = reflect(r.dir, r.normal), .o = r.o + r.dir * r.t,
							.t = MAX_DEPTH};
	float3			v = normalize(r.o - ref.o);

	ref.o += ref.dir * 0.09f;
	check_hit(&ref, d);
	if (ref.t < MAX_DEPTH){
		float ggx = tr_ggx(r.att.x * r.att.x, dot(r.normal, normalize(v + ref.dir)));
		final_col += (ref.color + lighting(ref, d)) * ggx;
	}
	else
		final_col = get_skybox_pixel(d.skybox, d.gid, ref.dir, d.res);
	return (final_col * r.att.y);
}
...
//	const int			skbx_enable = (int)info[15];
//	const int2			skbx_size = {(int)info[16], (int)info[17]};
	...
	if (r.t < MAX_DEPTH) {
		if (r.att.w == 1){
			...
			r.color += lighting(r, d);
	}
	else
		r.color = get_skybox_pixel(skybox, gid, r.dir, res);
	if (filter > 0)
